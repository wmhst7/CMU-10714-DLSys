...
class MultiHeadAttention(Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.causal = causal
        self.dropout = Dropout(dropout, device=device, dtype=dtype)
    ...
    def forward(
        ...
        # print("QKV SHAPE:", q.shape, k.shape, v.shape, k.transpose().shape)
        ### BEGIN YOUR SOLUTION
        ...
        if self.causal:
            ...
            probs = probs + mask.broadcast_to(probs.shape)
...
class AttentionLayer(Module):
    ...
    def forward(
        ...
        q = q.reshape(shape_q).permute((0, 2, 1, 3))
        shape_k = (batch_size, keys_values_len, self.num_head, self.dim_head)
        k = k.reshape(shape_k).permute((0, 2, 1, 3))
        v = v.reshape(shape_k).permute((0, 2, 1, 3))
        # 3
        x, att_mat = self.attn(q, k, v)
        x = x.permute((0, 2, 1, 3)).reshape((batch_size, queries_len, self.num_head * self.dim_head))
...
class TransformerLayer(Module):
    ...
    def forward(
        ...
        ### BEGIN YOUR SOLUTION
        # Attention 
        x = x + self.norm(x + self.dropout(self.attnlayer(x)))
