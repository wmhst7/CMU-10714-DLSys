...
class PowerScalar(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ### BEGIN YOUR SOLUTION
        return self.scalar * (node.inputs[0] ** (self.scalar - 1))
        ### END YOUR SOLUTION
...
class Permute(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ### BEGIN YOUR SOLUTION
        inv_axes = tuple(self.axes.index(i) for i in range(len(self.axes)))
...
class BroadcastTo(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ...
        # from back input_shape[i] == self.shape[i] or input_shape[i] == 1
        # broad_dims conlclues i if self.shape[i] != input_shape[i] 
        #         or i >= input_shape.len
        ...
        return out_grad.sum(tuple(broad_dims)).reshape(ori_shape)
        ### END YOUR SOLUTION
...
class MatMul(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ...
        if grad_a.shape != a:
            grad_a = grad_a.sum(tuple(list(range(len(grad_a.shape) - len(a.shape)))))
        if grad_b.shape != b:
            grad_b = grad_b.sum(tuple(list(range(len(grad_b.shape) - len(b.shape)))))
        return grad_a, grad_b
        ### END YOUR SOLUTION
