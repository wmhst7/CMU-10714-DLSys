...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        if self.is_compact() == False:
            raise ValueError("The NDArray is not compact")
        if prod(new_shape) != prod(self.shape):
            raise ValueError("New shape size is not equal to current shape")
    ...
    def __getitem__(self, idxs):
        ...
        new_strides = tuple([self.strides[i] * idxs[i].step for i in range(len(idxs))])
        new_offset = np.sum([self.strides[i] * idxs[i].start for i in range(len(idxs))])
        return self.make(shape=new_shape, strides=new_strides, device=self.device,
