...
void Compact(const AlignedArray& a, AlignedArray* out, std::vector<int32_t> shape,
  ...
  size_t cnt = 0, idx = 0;
  scalar_t dummy = 0;
  compactHelper(a, out, shape, strides, offset, cnt, idx, 0, 0, dummy);
  /// END SOLUTION
}
...
void EwiseSetitem(const AlignedArray& a, AlignedArray* out, std::vector<int32_t> shape,
  ...
  size_t cnt = 0, idx = 0;
  scalar_t dummy = 0;
  compactHelper(a, out, shape, strides, offset, cnt, idx, 0, 1, dummy);
  /// END SOLUTION
}
...
void ScalarSetitem(const size_t size, scalar_t val, AlignedArray* out, std::vector<int32_t> shape,
  ...
  /// BEGIN SOLUTION
  size_t cnt = 0, idx = 0;
  AlignedArray dummy(0);
  compactHelper(dummy, out, shape, strides, offset, cnt, idx, 0, 2, val);
  /// END SOLUTION
}
...
void ReduceMax(const AlignedArray& a, AlignedArray* out, size_t reduce_size) {
  ...
  for(size_t i = 0; i < a.size; i++){
    if(i % reduce_size == 0){
        out->ptr[i / reduce_size] = a.ptr[i];
    }
    out->ptr[i / reduce_size] = std::max(a.ptr[i], out->ptr[i / reduce_size]);
  }
  /// END SOLUTION
}
